...
use crate::ark_circom_mini::PairingEngine;
...
fn read_constraint_vec<R: Read, E: PairingEngine>(mut reader: R) -> Result<ConstraintVec<E>> {
    ...
    for _ in 0..n_vec {
        vec.push((
            ...
            <<E as Pairing>::ScalarField as PrimeField>::BigInt::deserialize_with_mode(&mut reader, Compress::No, Validate::No)
            .and_then(|b| Ok(E::ScalarField::from_bigint(b).unwrap())).unwrap(),
        ));
    }
    Ok(vec)
}
