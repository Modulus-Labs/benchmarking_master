...
#[cfg(feature= "circom-2")]
fn normalize(n: &BigInt, prime: &BigInt) -> BigInt {
    let mut res = n % prime;
    if res.sign() == Sign::Minus {res += prime;}
    res
}
...
impl WitnessCalculator {
    ...
    fn calculate_witness_circom2<I: IntoIterator<Item = (String, Vec<BigInt>)>>(
        ...
        for (name, values) in inputs.into_iter() {
            ...
            for (i, value) in values.into_iter().enumerate() {
                let f_arr = to_array32(&normalize(&value, prime), n32 as usize);
                for j in 0..n32 {
    ...
    >(
        ...
        let modulus = E::ScalarField::MODULUS;
